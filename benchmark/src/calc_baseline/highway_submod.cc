...
static int32_t ONE_delta_pixels_fdiv_submod(
    const DI di
  , const DW dw	
  , const DF df
...
)
{
	...
	auto v_x_one_row = mod_sub_v3(di, v_x_baseline, settings.refline_distance);
...
static int32_t hwy_calc_delta_pixels_fdiv_submod(
	  const size_t	size
  	...
  	, int32_t *delta_pixels )
{
	...
	*delta_pixels = 0;
	for (size_t i = 0; i < size; i += N) 
	{
		...
		valid_points += ONE_delta_pixels_fdiv_submod(di, di32, df, vsettings, x_screen + i, y_screen + i, &ONE_delta_px);
...
namespace deltapx {
	int32_t run_hwy_calc_delta_pixels_fdiv_submod(
  	...
  	, int32_t *delta_pixels )
	{
		return HWY_STATIC_DISPATCH(hwy_calc_delta_pixels_fdiv_submod)(size, x_screen, y_screen, settings, delta_pixels );
	}
