...
static int32_t ONE_delta_pixels_fdiv(
    const DI di
  , const DW dw	
  , const DF df
...
static int32_t hwy_calc_delta_pixels_fdiv(
	  const size_t	size
  	...
  	, int32_t *delta_pixels )
{
	...
	*delta_pixels = 0;
	for (size_t i = 0; i < size; i += N) 
	{
		...
		valid_points += ONE_delta_pixels_fdiv(di, di32, df, vsettings, x_screen + i, y_screen + i, &ONE_delta_px);
...
namespace deltapx {
	int32_t run_hwy_calc_delta_pixels_fdiv(
  	...
  	, int32_t *delta_pixels )
	{
		return HWY_STATIC_DISPATCH(hwy_calc_delta_pixels_fdiv)(size, x_screen, y_screen, settings, delta_pixels );
	}
