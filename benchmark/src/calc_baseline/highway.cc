...
static int32_t hwy_calc_delta_pixels_int16_fdiv(
	  const size_t	size
  	, const int16_t* __restrict x_screen
  	, const int16_t* __restrict y_screen
  	, const CalcSettings& settings
...
namespace deltapx {
	...
	int32_t run_hwy_calc_delta_pixels_int16_fdiv(
  	...
  	, int32_t *delta_pixels )
	{
		return HWY_STATIC_DISPATCH(hwy_calc_delta_pixels_int16_fdiv)(size, x_screen, y_screen, settings, delta_pixels );
	}
