...
public class BaselineSimpleOptionalImpl implements IBaselineCalcOptional {

    //
    // 0,0 ... fluchtpunkt
    //
    private CoordPoint project_screen_point_to_coord(final int x_point_screen, final int y_point_screen, final int x_half, final int rowPerspectivePx) 
    {
        return
            new CoordPoint(
    ...
    //
    private Integer project_x_onto_baseline(final CoordPoint point, final int y_baseline) //, int *x_base_line) 
    {
        if ( point.x == 0 ) 
        {
    ...
    private boolean is_within_range(int x, final int range_baseline)
    {
        return Math.abs(x) < range_baseline;
        //return (unsigned int)std::abs(x) < range_baseline;
    }

    private int project_x_inbetween_first_row(final int x_baseline, final int refline_distance) 
    {
    ...
    private int distance_to_nearest_refline_on_baseline(final int x_first_row, final int refline_distance, final int half_refline_distance)
    {
    ...
    @Override
    public Optional<Integer> DeltaToNearestRefLine(int x_screen, int y_screen, final CalcSettings settings) {
        CoordPoint p;
        ...
        return Optional.of(delta_pixels);
    }
}
